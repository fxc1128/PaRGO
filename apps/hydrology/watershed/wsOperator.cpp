...
/*
void streamOperator::demLayer(RasterLayer<double>& layerD) {
    _pDEMLayer = &layerD;
...
*/
void wsOperator::orderLayer(RasterLayer<double>& layerD) {
    _porderout = &layerD;
	//_pssaNbrhood = layerD.nbrhood();
...
bool wsOperator::Operator(const CellCoord& coord, bool operFlag) {
	...
	if(num==0)
	{
		...
		if(fabs(ssa[iRow][iCol] - _noData) > Eps&&ssa[iRow][iCol]>=1&&!fabs(dir[iRow][iCol]-_noData)<Eps&&dir[iRow][iCol]>=0)
	...
	if(num==1){
		if(fabs(ssa[iRow][iCol] - _noData) > Eps&&ssa[iRow][iCol]>=1){	
				...
				for (tCol = iCol - 1; tCol <= iCol + 1; tCol++) {
					if(!fabs(dir[tRow][tCol]-_noData)<Eps&&dir[tRow][tCol]>=0){
						...
						if (d1[d]+tRow==iRow && d2[d]+tCol==iCol&&d + dir[iRow][iCol] != 8&& d != 4 ) {
							...
							if(ssa[tRow][tCol]>=1)contribs[iRow][iCol]++;
						}
					}
				}
			}
				...
				for (tCol = iCol - 1; tCol <= iCol + 1; tCol++) {
					if(!fabs(dir[tRow][tCol]-_noData)<Eps&&dir[tRow][tCol]>=0){
						...
						if (d1[d]+tRow==iRow && d2[d]+tCol==iCol&&d + dir[iRow][iCol] != 8&& d != 4 ) {
							...
							if(ssa[tRow][tCol]==1)n1=n1+1;
						}
					}
				}
			}
			//if(ssa[iRow][iCol]==2)
			//	n1=n1+contribs[iRow][iCol]+1;
			//else
			n1=n1+1;
		}
		if (iRow == _maxrow && iCol == _maxcol) {
			cout<<"n1:"<<n1<<"rank:"<<rank<<endl;
			...
			for(int k=0;k<size;k++){
				if(rank==k){
					//if(k==0)
					//{
					//	MPI_Isend(&n1,1,MPI_INT,k+1,k,MPI_COMM_WORLD,&send_request);
					//	cout << "-> Process " << k << " sent " << n1 << " to Process"<< k+1 << endl; 
					//}else 
					if(k<size-1&&k>0){
					//	MPI_Irecv(&n2,1,MPI_INT,k,k,MPI_COMM_WORLD,&recv_request);
					//	cout << "<- Process 0 received " << n2 << " from Process " << k << endl; 
					//	n1=n2+n1;
					//}else{
						cout<<"k:"<<k<<endl;
						...
						cout << "-> Process " << k << " sent " << n1 << " to Process"<< k+1 << endl; 
						MPI_Irecv(&n2,1,MPI_INT,k,k,MPI_COMM_WORLD,&recv_request);
						cout << "<- Process "<<k+1<<" received " << n2 << " from Process " << k << endl; 
						n1=n2+n1;
						cout<<"pro"<<k<<"re_n1:"<<n1<<endl;
					}
				}
			}
			*/
